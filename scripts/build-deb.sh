#!/usr/bin/env bash
# build-deb.sh - reproducible .deb build wrapper for pam-certauth.
#
# Usage:
#   scripts/build-deb.sh [--allow-dirty] [--check-only] [--skip-lintian]
#
# Outputs ../pam-certauth_<ver>_amd64.deb, ../pam-certauth_<ver>_amd64.changes,
# and ../pam-certauth_<ver>_amd64.buildinfo.
...
if [[ $skip_lintian -eq 0 ]]; then
    ...
    DEB="../pam-certauth_${VER}_amd64.deb"
    if [[ ! -f "$DEB" ]]; then
...
echo "done: built ../pam-certauth_$(dpkg-parsechangelog -SVersion)_amd64.deb"
