...
#   * libpdp ipdp_set_label_*() on /run/pam_certauth/sessions.json
#
...
readonly OUT=${BENCH_OUT:-/var/log/pam_certauth/bench-mac.csv}

log() { printf '[%s] %s\n' "$TAG" "$*" >&2; }

require_helper() {
    if ! command -v pam-certauth-test >/dev/null 2>&1; then
        log "pam-certauth-test helper binary missing — bench requires it"
        log "TODO(helper-binary): build crates/pam_certauth_cli with --features bench-helper"
        exit 2
    fi
}
...
main() {
    ...
    local i
    for ((i = 0; i < N; i++)); do
        pam-certauth-test bench-open-close --csv >> "$OUT" || {
