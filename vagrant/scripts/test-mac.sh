...
#
# Each scenario:
#   1. Drops one of the prepared policy TOMLs into /etc/pam_certauth/config.toml.
...
#   - /etc/pam_certauth/test/leaves/<scenario>.{crt,key}.pem present
#   - /etc/pam_certauth/policies/*.toml from tests/fixtures/policy-*.toml
#   - pam-certauth ≥ 0.3.0 with astra-mac feature compiled in
#
...
readonly CFG=/etc/pam_certauth/config.toml
...
apply_policy() {
    ...
    local name=$1
    install -m 0640 -o root -g pamcertauth "$POLICIES/policy-${name}.toml" "$CFG"
    systemctl restart pam-certauth.service || true
...
expect_event() {
    ...
    local ev=$1 since=$2
    journalctl --since "$since" -u pam-certauth.service --no-pager \
...
t1_required_l2() {
    ...
    # TODO(helper-binary): replace with pam-certauth-test open_session once
...
#   TODO(helper-binary): requires pam-certauth-test with `open_session_bg` and
...
t11_concurrent_writes() {
    local tag=T11
    log "[$tag] SKIPPED — needs pam-certauth-test open_session_bg helper"
}
...
t12_monitord_restart() {
    ...
    sleep 2
    if systemctl is-active --quiet pam-certauth.service; then
...
main() {
    ...
    # Requires pam-certauth-test --leaf <path> --policy <toml> open_session.
    t3_required_no_ext_denies
    t4_optional_no_ext
    t5_optional_fallback
    t6_ignore
