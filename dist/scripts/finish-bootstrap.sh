#!/usr/bin/env bash
# finish-bootstrap.sh — flip pam-certauth from bootstrap to production layout.
#
...
#        b. Validates the new config (`pam-certauth check`).
#        c. Restarts `pam-certauth.service` and waits for it to become
#           active.
#        d. Dumps the per-host identity TSV to a USB stick (with retries)
#           or to `/var/lib/pam_certauth/` as a fallback.
...
Flip pam-certauth config from bootstrap to production layout and dump the
...
# Config presence.
if [ ! -f "$CONFIG" ]; then
    echo "ERROR: $CONFIG not found — is pam-certauth installed?" >&2
    exit 1
fi
...
# Validate.
echo "==> Validating new config (pam-certauth check)..."
if ! pam-certauth check --config "$CONFIG"; then
    echo "ERROR: pam-certauth check failed on new config." >&2
...
if [ "$DO_RESTART" -eq 1 ]; then
    echo "==> Restarting pam-certauth.service..."
    systemctl restart pam-certauth.service
    # Wait up to 30s for active state.
    WAITED=0
    while [ "$WAITED" -lt 30 ]; do
        state="$(systemctl is-active pam-certauth.service 2>/dev/null || true)"
    ...
    if [ "$state" != "active" ]; then
        echo "ERROR: pam-certauth.service did not become active within 30s (current: ${state:-unknown})." >&2
        echo "       Investigate with: journalctl -u pam-certauth.service -n 100" >&2
        exit 1
    fi
else
...
if [ "$DO_DUMP" -eq 1 ]; then
    ...
    while [ "$WAITED" -lt 60 ]; do
        if pam-certauth dump-host-id --usb 2>/tmp/pam-certauth-dump.err; then
            USB_OK=1
            break
        fi
    ...
    if [ "$USB_OK" -eq 1 ]; then
        ...
        rm -f /tmp/pam-certauth-dump.err
    else
        echo "    No USB stick after 60s; falling back to local file."
        mkdir -p /var/lib/pam_certauth
        FALLBACK="/var/lib/pam_certauth/host-ids-${HOSTNAME_VAL}-${UTC_STAMP}.tsv"
        if ! pam-certauth dump-host-id --output "$FALLBACK"; then
            echo "ERROR: dump-host-id --output failed." >&2
            cat /tmp/pam-certauth-dump.err >&2 2>/dev/null || true
            rm -f /tmp/pam-certauth-dump.err
            exit 1
        fi
        rm -f /tmp/pam-certauth-dump.err
...
echo
echo "================================================================"
echo " pam-certauth: bootstrap flip complete"
echo "================================================================"
echo "  config:        $CONFIG"
echo "  backup:        $BACKUP"
