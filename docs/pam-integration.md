# Интеграция `pam_certauth` в `/etc/pam.d/*`
...
- **Если в файле есть `auth ... pam_parsec_mac.so`** (типично для Astra
  ...
  **после** этой строки. Иначе snippet `certauth-only` с `success=done`
...
Поставочные snippets (`certauth`, `certauth-only`, `certauth-optional`)
...
sudo /usr/share/pam-certauth/integrate-pam.sh --mode=<режим> /etc/pam.d/login
```
...
сессию. Без интеграции `pam_certauth` в `/etc/pam.d/fly-dm`
...
1. **Точка входа в сессию.** МКЦ-метка (`pam_cert_max_integrity ∩ МНКЦ
   ...
   `pam_certauth`, метка не выставится.
...
3. **Hot-plug до логина.** `fly-dm` стартует раньше пользовательских
   сервисов; `pam-certauth.service` обязан быть `Before=fly-dm.service`
...
[`dist/pam.d/certauth`](../dist/pam.d/certauth)): если модуль успешно
...
2. **Альтернативный путь логина**, который НЕ проходит через
   ...
   админ-аккаунта без `@include certauth`. Иначе потеря или
...
**МКЦ включён** — `auth required pam_parsec_mac.so` + `@include certauth`
...
```bash
sudo pam-certauth check    # ловит pam_stack_session_misorder и др.
```
...
- **SysV init-скрипт** `/etc/init.d/pam-certauth` — для non-systemd
...
`/run/pam_certauth/pam-certauth.pid` и читает
...
- На systemd-хостах править SysV-скрипт не требуется — авторитативный
  источник конфигурации службы — `pam-certauth.service`.

## 11. См. также

- [install.md](install.md) — установка `pam_certauth` целиком.
