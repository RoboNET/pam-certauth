# Troubleshooting `pam_certauth`

Единый справочник по диагностике. Разделы:

- [§1 Cert/auth-ошибки](#1-certauth-ошибки)
...
```bash
sudo journalctl -u pam-certauth --since '5 min ago'
sudo journalctl -t pam_certauth | tail -50
...
В журнале искать `pam_certauth.auth.fail.<reason>`. Список причин —
...
**By-design.** `pam_certauth` — жёсткий второй фактор: без валидного
...
`pam_certauth`-цепочки) до валидации развёртывания.
...
- `config.toml` повреждён → запустить вручную:
  `sudo /usr/bin/pam-certauth` и читать диагностический вывод.

### monitord не запускается

**Симптом:** `systemctl status pam-certauth` показывает `failed`.
...
- занятый сокет: `lsof /run/pam_certauth/monitord.sock`;
- нет прав на `/run/pam_certauth/`: `ls -la`, должно быть `0750 root:root`;
- повреждённый `config.toml`: запустить вручную `sudo /usr/bin/pam-certauth`;
...
### `pam-certauth` в `/etc/pam.d/login` не находится
...
- Файл отсутствует → `dpkg -l pam-certauth`. Возможно прерванная
...
INFO pam_certauth.monitord: tip: pam_sm_open_session pushes XDG_SESSION_ID ...
```
...
**Симптом:** `pam_certauth` отработал успешно, но через несколько
...
`@include certauth-only` оказался ПЕРЕД `auth required
pam_parsec_mac.so`. `certauth-only` использует
`auth [success=done default=die] pam_certauth.so` — `success=done`
...
Если номер строки `@include certauth*` **меньше** номера
...
**Причина 3:** МКЦ-ядро включено, но `bfs_service` не имеет MAC-уровня.
...
```bash
sudo systemctl restart pam-certauth     # перерисует banner
...
- **`machine_id` пустой** — очищен перед клонированием, systemd не
  ...
  `systemd-machine-id-setup && systemctl restart pam-certauth`.
...
- использовать fallback в `/var/lib/pam_certauth/`.

### `active_under_current_config=no` для всех строк

Бывает если `--sources` указали несуществующие источники (опечатка).
`pam-certauth check` обычно ловит, но если прошло — проверить
`[host_identity].sources` в `config.toml`.

### Bootstrap-cert отвергается на клоне

- Trust anchor не попал в образ: `pam-certauth check` покажет
...
3. Обновить CRL на endpoints (см. [operations.md §2.2](operations.md);
   ускоренная процедура — `systemctl start pam-certauth-crl-update.service`).
4. Проверить журнал:
   ```bash
