...
### 2.4 `pam_certauth_monitord` (бинарь `pam-certauth-monitord`)
...
`/usr/sbin/pam-certauth-monitord` и поставляется юнитом
...
flowchart TD
    ...
    monitord[pam-certauth-monitord] --> proto
    monitord --> core
...
(см. [`pam-certauth-monitord.service`](../dist/systemd/pam-certauth-monitord.service)
...
- Реализация: [`crates/pam_certauth_monitord/src/peercred.rs`](../crates/pam_certauth_monitord/src/peercred.rs).
