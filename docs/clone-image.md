...
> Документы рядом:
> - [install.md](install.md) — пошаговая установка `pam_certauth` (выполняется на эталоне).
...
Сертификат `pam_certauth` привязан к `host_id_hash` АРМ-а
...
./issue-service-cert.sh --mode bootstrap \
    --ca-dir /etc/ssl/pki \
    --out-dir /etc/pam_certauth/bootstrap
```
...
```toml
# /etc/pam_certauth/config.toml (фрагмент)
...
- остановить `pam-certauth.service` (`systemctl stop pam-certauth`);
- очистить `/var/lib/pam_certauth/sessions.json` (опционально, не критично);
...
1. **Rewrite `config.toml`**:
   ...
   - Backup → `/etc/pam_certauth/config.toml.bak.<UTC-ISO8601>`.
2. **Валидирует** новый конфиг: `pam-certauth check`. Если ERROR —
   rollback бекапа, exit ≠ 0.
3. **Рестарт** `pam-certauth.service`, ждёт `is-active=active` до 30 с.
4. **Снимает дамп**: `pam-certauth dump-host-id --usb` с ретраями
   (~30 с на появление USB). Fallback: TSV в
   `/var/lib/pam_certauth/host-ids-<hostname>-<UTC>.tsv`.
...
```
pam-certauth-admin-tools-<ver>.tar.gz
```
...
- `pam_cert_user_binding = bfs_service`;
...
```bash
journalctl -u pam-certauth -g 'host_identity: probe' -n 20
journalctl -u pam-certauth -g 'host_binding' -n 20
```
...
- name: Finish bootstrap on cloned ATM
  ansible.builtin.command:
    cmd: /usr/share/pam-certauth/finish-bootstrap.sh --non-interactive --no-dump
  register: finish
  changed_when: "'no changes' not in finish.stdout"

- name: Fetch host_id dump
  ansible.builtin.command:
    cmd: pam-certauth dump-host-id --output /tmp/host-ids.tsv
  changed_when: false
