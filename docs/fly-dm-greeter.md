...
`pam_certauth` отправляет короткую идентификацию машины в PAM в
...
syslog: `journalctl -t pam_certauth | grep host_identity`. Цель —
...
На Astra с МКЦ-3 (production-банкоматы) `fly-dm` под темой
...
```bash
sudo journalctl -u pam-certauth -g fly_dm_greeter -n 20
```
