...
Начиная с версии 1.0.2 сертификат сам определяет область своего действия:
отдельный подписанный TOML-файл host-binding и поле ролей удалены.
Авторизация «какой пользователь на каком хосте» закодирована в двух
X.509-расширениях, которые проверяет PAM-модуль на этапе аутентификации:
