...
- `/usr/share/pam-certauth/finish-bootstrap.sh` — single-pass переход
  с bootstrap-state на production: rewrite `config.toml`
  (`sources = ["override"]` → `["dmi_board_serial", "machine_id"]`),
  `pam-certauth check`, restart daemon, дамп host_id'ов на USB.
...
`pam-certauth check` ловит ошибки порядка PAM-стека (например
...
Пакет ставит **оба** init-варианта: `pam-certauth.service` (systemd)
и `/etc/init.d/pam-certauth` (SysV). На systemd-хостах SysV-скрипт
...
2. `usercaps -m "+3" pamcertauth` + `pdpl-user --ilevel 63 pamcertauth`.
3. Скопировать `pam-certauth.example` и `mac-integrity.conf.example`
   из `/usr/share/pam-certauth/` в `/etc/pam.d/` и
   `/etc/systemd/system/pam-certauth.service.d/`.
