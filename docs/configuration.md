...
- `/etc/pam_certauth/config.toml` — основная конфигурация модуля и
  демона `pam-certauth-monitord`.

Авторизация «какой пользователь на каком хосте» начиная с 1.0.2
живёт в самом сертификате (X.509-расширения `pam_cert_host_binding`
и `pam_cert_user_binding`). См. [docs/cert-issuance.md](cert-issuance.md).
