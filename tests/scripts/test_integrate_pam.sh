...
#   - session required pam_certauth.so is added AFTER @include common-session;
...
grep -q '^@include certauth$' "$WORK/sudo" \
    || { echo "FAIL: @include not added" >&2; exit 1; }
grep -qE '^session[[:space:]]+required[[:space:]]+pam_certauth\.so' "$WORK/sudo" \
...
cert_ses=$(line_of '^session[[:space:]]+required[[:space:]]+pam_certauth\.so' "$WORK/sudo")
test -n "$unix_ses" && test -n "$cert_ses" && [ "$cert_ses" -gt "$unix_ses" ] \
    || { echo "FAIL: certauth session ($cert_ses) must come after pam_unix session ($unix_ses)" >&2; exit 1; }
...
test "$first_match" = "@include certauth" \
...
if grep -qE '^@include certauth(-optional|-only)?$' "$WORK/sudo"; then
    echo "FAIL: --unintegrate did not remove @include" >&2
    exit 1
fi
if grep -qE '^[[:space:]]*session[[:space:]]+required[[:space:]]+pam_certauth\.so' "$WORK/sudo"; then
...
grep -qE '^[[:space:]]*session[[:space:]]+required[[:space:]]+pam_certauth\.so' "$WORK/sudo" \
...
include=$(line_of '^@include certauth-only$' "$WORK/login_astra")
common_ses=$(line_of '^@include[[:space:]]+common-session([[:space:]]|$)' "$WORK/login_astra")
cert_ses=$(line_of '^session[[:space:]]+required[[:space:]]+pam_certauth\.so' "$WORK/login_astra")
