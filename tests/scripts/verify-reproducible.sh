#!/usr/bin/env bash
# verify-reproducible.sh - build pam-certauth twice and compare.
#
...
build_into() {
    local dest="$1"
    cargo clean
    rm -f ../pam-certauth_*.deb ../pam-certauth_*.changes ../pam-certauth_*.buildinfo
    scripts/build-deb.sh --skip-lintian --allow-dirty
    cp ../pam-certauth_*.deb "$dest"/
}
