//! Helpers shared across the GOST integration tests in `pam_certauth_core`.
//!
...
pub fn skip_unless_gost_ready() -> bool {
    ...
    if !pam_certauth_core::gost::engine::is_available_after_attempt(None) {
...
#[must_use]
pub fn load_pem_cert(name: &str) -> pam_certauth_core::x509::Certificate {
    let path = fixture_path(name);
    let bytes =
        std::fs::read(&path).unwrap_or_else(|e| panic!("read fixture {}: {e}", path.display()));
    pam_certauth_core::x509::Certificate::from_pem(&bytes)
