...
//! [`pam_certauth_core::self_check::self_check`] in stage 4: when
...
use pam_certauth_core::self_check::self_check;
use pam_certauth_core::SelfCheckError;
...
fn pkcs11_mode_with_missing_module_path_returns_module_missing() {
    ...
    let cfg = cfg_with(
        "pkcs11",
        "/nonexistent/__pam_certauth_no_such_module__.so",
        &anchor,
    );
