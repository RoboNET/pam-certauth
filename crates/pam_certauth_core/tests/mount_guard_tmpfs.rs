...
use pam_certauth_core::error::MountGuardError;
use pam_certauth_core::mount_guard::{MountFlags, MountGuard, MountOps};
...
fn mount_guard_real_ops_placeholder_lifecycle() {
    ...
    {
        let ops = std::sync::Arc::new(pam_certauth_core::mount_guard::RealMountOps);
        let _guard =
            pam_certauth_core::mount_guard::MountGuard::new_tmpfs(ops, tmp.path(), "abc-test")
                .unwrap_or_else(|err| panic!("guard: {err}"));
    }
