...
use pam_certauth_core::pkcs12::LoadedKeyMaterial;
...
#[test]
fn dispatches_rsa() {
    let m = LoadedKeyMaterial::from_p12(RSA, &SecretString::from("correct-pin".to_string())).unwrap();
...
#[test]
fn dispatches_ecdsa() {
    let m = LoadedKeyMaterial::from_p12(ECDSA, &SecretString::from("correct-pin".to_string())).unwrap();
    let priv_k = m.private_key().unwrap();
...
fn mismatched_key_yields_bad_signature() {
    ...
    let m_rsa = LoadedKeyMaterial::from_p12(RSA, &SecretString::from("correct-pin".to_string())).unwrap();
    let m_ec = LoadedKeyMaterial::from_p12(ECDSA, &SecretString::from("correct-pin".to_string())).unwrap();
