...
# `tests/gost_*_real.rs` integration tests in `pam_certauth_core`.
#
# Prerequisites:
...
openssl req -x509 -engine gost -newkey gost2012_256 \
    ...
    -subj "/CN=pam_certauth Test GOST CA 256" \
    -md_gost12_256 \
...
openssl req -x509 -engine gost -newkey gost2012_512 \
    ...
    -subj "/CN=pam_certauth Test GOST CA 512" \
    -md_gost12_512 \
    -config <(printf '[req]\ndistinguished_name=dn\n[dn]\n')
...
openssl req -engine gost -newkey gost2012_256 \
    ...
    -out gost_ee_256.csr \
    -subj "/CN=pam_certauth-test-ee-256" \
    -config <(printf '[req]\ndistinguished_name=dn\n[dn]\n')
...
openssl req -engine gost -newkey gost2012_512 \
    ...
    -out gost_ee_512.csr \
    -subj "/CN=pam_certauth-test-ee-512" \
    -config <(printf '[req]\ndistinguished_name=dn\n[dn]\n')
openssl x509 -req -engine gost \
    -in gost_ee_512.csr \
