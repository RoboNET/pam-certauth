...
//! `crates/pam_certauth/tests/auth_e2e_pkcs11.rs` cover the full
...
fn module_load_with_nonexistent_path_returns_module_path_missing() {
    let nope = PathBuf::from("/nonexistent/__pam_certauth_no_such_module__.so");
...
fn live_backend_finds_slot_with_token() {
    ...
    let slot = backend
        ...
        .expect("find_slot");
    let serial =
        pam_certauth_core::token::pkcs11::read_token_serial(&backend, slot).expect("read serial");
...
fn live_three_wrong_pins_yield_max_attempts() {
    ...
    let prompter = |_p: &str| -> Result<SecretString, pam_certauth_core::pam_conv::PamConvError> {
        prompts += 1;
...
fn live_cert_lookup_with_unknown_label_returns_not_found() {
    ...
    let session =
        match pam_certauth_core::token::pkcs11::Pkcs11Session::open(&backend, slot, &user_pin()) {
            Ok(s) => s,
    ...
    let err = session
        .find_certificate(Some("__pam_certauth_no_such_label__"))
        .err()
        .expect("must fail");
    assert!(
