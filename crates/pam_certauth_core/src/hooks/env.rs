...
//! `PAM_CERTAUTH_*` variable from [`HookVars`], then renders user-supplied
...
pub fn build_env_vector(
    ...
    // 2. PAM_CERTAUTH_* — empty string for None so hooks can rely on the
    //    keys being present.
    env.insert("PAM_CERTAUTH_STAGE".into(), hook.stage.to_string());
    env.insert(
        "PAM_CERTAUTH_USER".into(),
        vars.pam_user.clone().unwrap_or_default(),
    );
    env.insert(
        "PAM_CERTAUTH_SERVICE".into(),
        vars.pam_service.clone().unwrap_or_default(),
    );
    env.insert(
        "PAM_CERTAUTH_HOST_ID".into(),
        vars.host_id.clone().unwrap_or_default(),
    );
    env.insert(
        "PAM_CERTAUTH_HOST_ID_HASH".into(),
        vars.host_id_hash.clone().unwrap_or_default(),
    );
    env.insert(
        "PAM_CERTAUTH_HOST_ID_SOURCE".into(),
        vars.host_id_source.clone().unwrap_or_default(),
    );
    env.insert(
        "PAM_CERTAUTH_CERT_CN".into(),
        vars.cert_cn.clone().unwrap_or_default(),
    );
    env.insert(
        "PAM_CERTAUTH_CERT_SERIAL".into(),
        vars.cert_serial.clone().unwrap_or_default(),
    );
    env.insert(
        "PAM_CERTAUTH_USB_SERIAL".into(),
        vars.usb_serial.clone().unwrap_or_default(),
    );
    env.insert(
        "PAM_CERTAUTH_USB_VID_PID".into(),
        vars.usb_vid_pid.clone().unwrap_or_default(),
    );
    env.insert(
        "PAM_CERTAUTH_SESSION_ID".into(),
...
mod tests {
    ...
    #[test]
    fn pam_certauth_set_for_some_and_empty_for_none() {
        ...
        let envv = build_env_vector(&hook, &vars, None).expect("build env");
        let envv = collect(&envv);
        assert_eq!(entry(&envv, "PAM_CERTAUTH_USER"), Some("alice"));
        assert_eq!(entry(&envv, "PAM_CERTAUTH_SERVICE"), Some("login"));
        assert_eq!(entry(&envv, "PAM_CERTAUTH_HOST_ID"), Some("hid"));
        assert_eq!(entry(&envv, "PAM_CERTAUTH_CERT_CN"), Some("CN"));
        assert_eq!(entry(&envv, "PAM_CERTAUTH_HOST_ID_HASH"), Some(""));
        assert_eq!(entry(&envv, "PAM_CERTAUTH_HOST_ID_SOURCE"), Some(""));
        assert_eq!(entry(&envv, "PAM_CERTAUTH_CERT_SERIAL"), Some(""));
        assert_eq!(entry(&envv, "PAM_CERTAUTH_USB_SERIAL"), Some(""));
        assert_eq!(entry(&envv, "PAM_CERTAUTH_USB_VID_PID"), Some(""));
        assert_eq!(entry(&envv, "PAM_CERTAUTH_SESSION_ID"), Some(""));
        assert_eq!(
            entry(&envv, "PAM_CERTAUTH_STAGE"),
