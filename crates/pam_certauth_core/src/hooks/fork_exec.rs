...
impl HookExecutor for ForkExecExecutor {
    fn execute(&self, hook: &HookConfig, vars: &HookVars) -> Result<HookOutcome, HookError> {
        ...
        tracing::info!(
            target: "pam_certauth.hook.start",
            stage = %stage,
            command = ?command,
            "hook starting",
        );
...
fn supervise_parent(args: SuperviseArgs) -> Result<HookOutcome, HookError> {
    ...
    let stdout_handle = spawn_reader("pam_certauth.hook.stdout", Arc::clone(&stdout_state))?;
    ...
    if killed_by_timeout {
        ...
        tracing::warn!(
            target: "pam_certauth.hook.timeout",
            stage = %stage,
            command = ?command,
            duration_ms,
            timeout_ms,
        ...
        tracing::info!(
            target: "pam_certauth.hook.finish",
            stage = %stage,
            command = ?command,
            exit_code,
            duration_ms,
