...
mod tests {
    ...
    fn finds_p12_with_custom_pattern_in_root() {
        let dir = tempfile::tempdir().unwrap();
        write_file(&dir.path().join("bfs_service.p12"), b"PFX2");
        let d = discover_credentials(dir.path(), "bfs_service.p12", "alice").unwrap();
        assert_eq!(d.p12_bytes, b"PFX2");
        assert!(d.p12_path.ends_with("bfs_service.p12"));
    }
    ...
    fn errors_when_custom_pattern_file_missing() {
        let dir = tempfile::tempdir().unwrap();
        let err = discover_credentials(dir.path(), "bfs_service.p12", "alice").unwrap_err();
        match err {
            DiscoveryError::P12NotFound { path } => {
                assert_eq!(path, PathBuf::from("bfs_service.p12"));
            }
            other => panic!("unexpected: {other:?}"),
        }
    }
