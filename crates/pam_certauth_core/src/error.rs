...
pub enum IpcError {
    ...
    #[error(transparent)]
    Encode(#[from] pam_certauth_proto::FramingError),
    /// New wire-layer error (`encode_message` / `decode_line`).
    #[error(transparent)]
    Wire(#[from] pam_certauth_proto::WireError),
    /// JSON decode failed.
    #[error("decode: {0}")]
    ...
    Protocol {
        /// Code.
        code: pam_certauth_proto::ServerErrorCode,
        /// Message.
        message: String,
    },
    ...
    Server {
        /// Numeric code (see [`pam_certauth_proto::error_codes`]).
        code: u32,
