...
//! the `pam_certauth` cdylib (where `unsafe_code` is permitted and the
...
/// driver in `pam_certauth::pam_conv`; tests typically only ever observe
...
/// reach for `pam_certauth::pam_conv::prompt_pin` instead, which talks to the
/// live PAM handle.
///
/// # Errors
///
