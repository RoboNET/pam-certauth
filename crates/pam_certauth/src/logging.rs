...
//! `pam_certauth`). On systems running journald the entries land in the
...
pub fn init_once() {
    INIT.get_or_init(|| {
        if let Err(err) = install_subscriber() {
            ...
            eprintln!("pam_certauth: failed to install tracing subscriber: {err}");
        }
    });
}
