...
fn config_path_from_args(args: &BTreeMap<String, String>) -> PathBuf {
    args.get("config").map_or_else(
        || PathBuf::from("/etc/pam_certauth/config.toml"),
        PathBuf::from,
    )
}

/// Map a PAM_TTY string into a [`pam_certauth_proto::SessionTarget`].
///
...
#[cfg(target_os = "linux")]
fn parse_pam_tty(tty: Option<&str>) -> pam_certauth_proto::SessionTarget {
    match tty {
        None => pam_certauth_proto::SessionTarget::Unknown,
        Some(s) if s.starts_with(':') => pam_certauth_proto::SessionTarget::display(s),
        Some(s) => pam_certauth_proto::SessionTarget::tty(s),
    }
}
...
pub unsafe extern "C" fn pam_sm_authenticate(
    ...
    crate::panic_guard::run_pam(|| {
        ...
        let cfg = match pam_certauth_core::config::load_validated_config(&cfg_path) {
            Ok(c) => c,
            Err(err) => {
                tracing::error!(target: "pam_certauth.auth", error = %err, "config load failed");
                return PAM_AUTHINFO_UNAVAIL;
            }
        };
        if let Err(err) = pam_certauth_core::self_check::self_check(&cfg) {
            tracing::error!(target: "pam_certauth.auth", error = %err, "self-check failed");
            return PAM_AUTHINFO_UNAVAIL;
        }
        ...
        let pam_user = match unsafe { crate::pam_helpers::pam_get_user_string(pamh) } {
            Ok(s) => s,
            Err(err) => {
                tracing::warn!(target: "pam_certauth.auth", error = %err, "pam_get_user failed");
                return PAM_AUTH_ERR;
            }
        };
        ...
        let (host_id_source, _host_id_raw, host_id_hash) = match crate::resolve_host_identity(&cfg)
        {
            Ok(t) => t,
            Err(err) => {
                tracing::error!(target: "pam_certauth.auth", error = %err, "host identity unresolved");
                return PAM_AUTHINFO_UNAVAIL;
            }
        };
        ...
        let wired = match crate::di::wire(cfg) {
            Ok(w) => w,
            Err(err) => {
                tracing::error!(target: "pam_certauth.auth", error = %err, "wiring failed");
                return PAM_AUTHINFO_UNAVAIL;
            }
        };
        ...
        let session_id = match fresh_session_id() {
            Ok(s) => s,
            Err(err) => {
                tracing::error!(target: "pam_certauth.auth", error = %err, "OS RNG unavailable; cannot mint session id");
                return PAM_AUTHINFO_UNAVAIL;
            }
        };
        let usb_wait = wired.cfg.usb_wait;
        let mountpoint_base = PathBuf::from("/run/pam_certauth/mounts");
        ...
        let hook_executor = pam_certauth_core::hooks::ForkExecExecutor::new();
        ...
        match outcome {
            ...
            Err(err) => {
                tracing::warn!(target: "pam_certauth.auth", error = %err, "authentication failed");
                err.pam_code()
            }
        }
    })
...
pub unsafe extern "C" fn pam_sm_open_session(
    ...
    crate::panic_guard::run_pam(|| {
        ...
        let cfg = match pam_certauth_core::config::load_validated_config(&cfg_path) {
            Ok(c) => c,
            Err(err) => {
                tracing::error!(target: "pam_certauth.session", error = %err, "config load failed");
                return PAM_AUTHINFO_UNAVAIL;
            }
        };
        ...
        match pam_certauth_core::hooks::run_hooks_for_stage(
            &cfg,
            pam_certauth_core::hooks::HookStage::SessionOpen,
            &executor,
            &vars,
        ) {
            Ok(()) => PAM_SUCCESS,
...
pub unsafe extern "C" fn pam_sm_close_session(
    ...
    crate::panic_guard::run_pam(|| {
        ...
        let cfg = match pam_certauth_core::config::load_validated_config(&cfg_path) {
            Ok(c) => c,
            Err(err) => {
                tracing::error!(target: "pam_certauth.session", error = %err, "config load failed (close)");
                return PAM_SUCCESS;
            }
        };
        ...
        if let Some(ctx) = unsafe { crate::data_handle::get_auth_context(pamh) } {
            ...
            let vars = pam_certauth_core::hooks::HookVars::for_session_close(&pam_user, ctx);
            let executor = pam_certauth_core::hooks::ForkExecExecutor::new();
            ...
            if let Err(err) = pam_certauth_core::hooks::run_hooks_for_stage(
                &cfg,
                pam_certauth_core::hooks::HookStage::SessionClose,
                &executor,
                &vars,
            ) {
                tracing::error!(
