...
//! the real [`pam_certauth_core::hooks::ForkExecExecutor`] and assert that
...
use pam_certauth::flow::{authenticate, Deps, InMemoryFlowIo};
use pam_certauth_core::config::ValidatedConfig;
use pam_certauth_core::hooks::{ForkExecExecutor, HookConfig, HookStage, OnFailure, RunAs};
use pam_certauth_core::host_identity::HostIdSourceKind;
use pam_certauth_core::ipc::StubClient;
use pam_certauth_core::x509::Certificate;
...
fn pre_auth_hook_runs_and_writes_marker_file() {
    ...
    cfg.hooks = vec![HookConfig {
        ...
        env: {
            ...
            // executor still inserts PAM_CERTAUTH_* automatically; we
            // rely on a custom MARKER var to find the temp path.
            m.insert(
                "MARKER".to_string(),
                pam_certauth_core::hooks::Template::parse(marker.to_string_lossy().as_ref())
                    .unwrap(),
            );
            m
        },
