...
use pam_certauth_monitord::server::perform_handshake;
use pam_certauth_proto::wire::encode_message;
use pam_certauth_proto::{ClientMessage, ServerMessage, PROTOCOL_VERSION};
...
async fn handshake_rejects_mismatched_version() {
    ...
    assert!(matches!(
        resp,
        ServerMessage::Error { code, .. } if code == pam_certauth_proto::error_codes::PROTOCOL_MISMATCH
    ));
