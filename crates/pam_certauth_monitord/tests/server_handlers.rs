...
use pam_certauth_monitord::state::OnUsbRemoved;
use pam_certauth_monitord::testing::spawn_test_server_with;
use pam_certauth_monitord::udev_query::{AlwaysAbsent, AlwaysPresent};
...
async fn session_open_returns_device_gone_when_absent() {
    ...
    assert!(
        matches!(err, pam_certauth_core::error::IpcError::DeviceGone),
        "got {err:?}"
    );
    server.shutdown_and_join().await;
}
