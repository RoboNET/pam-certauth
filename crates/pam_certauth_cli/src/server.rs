...
pub enum ServerError {
    ...
    #[error("encode: {0}")]
    Encode(pam_certauth_proto::WireError),
                    ...
                    Ok((stream, _)) => {
                        if cfg.enforce_peercred {
                            if let Err(e) = verify_peer_credentials(&stream) {
                                tracing::warn!(target: "pam_certauth.monitord", error = %e, "peer rejected");
                                continue;
                            }
                        }
                        ...
                        tokio::spawn(async move {
                            ...
                            if let Err(e) = serve_connection_with(stream, event_tx, idle_timeout).await {
                                tracing::debug!(target: "pam_certauth.monitord", error = %e, "connection ended");
                            }
                        });
                    }
                    Err(e) => {
                        tracing::warn!(target: "pam_certauth.monitord", error = %e, "accept failed");
                    }
                }
            }
        }
...
pub async fn serve_connection_with(
    ...
    loop {
        ...
        let msg = match decode_line::<ClientMessage>(line) {
            Ok(m) => m,
            Err(e) => {
                tracing::warn!(target: "pam_certauth.monitord", error = %e, "decode failed, closing");
                let bytes = encode_message(&ServerMessage::Error {
                    code: error_codes::BAD_REQUEST,
                    message: format!("{e}"),
                })
...
pub async fn perform_handshake(mut stream: UnixStream) -> Result<UnixStream, ServerError> {
    ...
    let msg: ClientMessage = decode_line(&buf).map_err(|e| match e {
        pam_certauth_proto::WireError::Decode(e) => ServerError::Decode(e),
