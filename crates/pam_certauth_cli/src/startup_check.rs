...
//! 4. World-writable bits on `/etc/pam_certauth/ca/`.
...
//!
//! The same pipeline is exposed via the `pam-certauth check` subcommand so
...
impl StartupCheckReport {
    ...
    /// `pam_certauth.startup_check` so an operator can grep
    /// `journalctl -t pam-certauth -g startup_check`.
    pub fn log(&self) {
        for r in &self.records {
            match r.severity {
                StartupCheckSeverity::Info => {
                    tracing::info!(
                        target: "pam_certauth.startup_check",
                        check = r.check,
                        "{}",
                        r.message
                    );
...
pub struct StartupCheckOptions {
    ...
    /// (currently only `/etc/pam_certauth/ca/`). `None` means "use the real
    /// host root".
...
pub fn real_kernel_parsec_probe() -> KernelParsecState {
    ...
    {
        use pam_certauth_core::mac::backend::MacRuntime;
        // Cheap, side-effect free; documented to be safe in any state.
        match pam_certauth_core::mac::ffi::probe_runtime() {
