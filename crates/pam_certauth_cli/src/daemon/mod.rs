...
//! the `pam-certauth-monitord` binary. As part of the Phase 0 pre-flight for
//! scopes / M-of-N (see `docs/superpowers/plans/2026-02-06-scopes-and-m-of-n.md`)
//! the binary was renamed to `pam-certauth` and grew clap subcommands; this
...
/// shipped systemd unit (`ExecStart=/usr/bin/pam-certauth daemon
...
pub fn run(args: DaemonArgs) -> ExitCode {
    ...
    match rt.block_on(run_async(args)) {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            eprintln!("pam-certauth daemon: {e:#}");
            ExitCode::FAILURE
        }
    }
}
