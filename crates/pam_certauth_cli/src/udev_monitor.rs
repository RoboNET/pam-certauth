        ...
        .spawn(move || {
            while !shutdown.is_cancelled() {
                if let Err(e) = run_monitor(&tx, &shutdown) {
                    tracing::warn!(target: "pam_certauth.monitord", error = %e, "udev monitor failed, retrying in 1s");
                    std::thread::sleep(std::time::Duration::from_secs(1));
                }
            }
        })
        .unwrap_or_else(|e| {
            tracing::error!(target: "pam_certauth.monitord", error = %e, "failed to spawn udev thread");
            std::thread::spawn(|| {})
        })
}
