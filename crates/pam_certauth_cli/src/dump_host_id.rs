//! `pam-certauth dump-host-id` subcommand.
//!
...
//!    `machine_id`, …).
//! 3. `systemctl restart pam-certauth`.
//! 4. Operator runs `sudo pam-certauth dump-host-id --usb`.
...
use pam_certauth_core::config::validated::HostIdentitySection;
use pam_certauth_core::host_identity::chain::{ProbeResult, ResolvedHostId};
use pam_certauth_core::host_identity::{normalize_host_id, HostIdSourceKind, HostIdentityResolver};
use pam_certauth_core::usb::{UdevEnumerator, UsbDevice, UsbEnumerator};
use sha2::{Digest, Sha256};

/// CLI arguments for `pam-certauth dump-host-id`.
#[derive(Debug, Args)]
pub struct DumpHostIdArgs {
    /// Path to `config.toml`. Defaults to `/etc/pam_certauth/config.toml`,
    /// matching the daemon and `pam-certauth check`.
    ...
    /// `/run/pam_certauth/host-id-dump/`, write
...
pub fn render_tsv(
    ...
    let _ = writeln!(out, "# pam-certauth host identity probe — {hostname} — {ts}");
...
fn pick_usb_partition() -> Result<UsbDevice, DumpError> {
    use pam_certauth_core::mount::usb::ALLOWED_FS;
...
/// `/run/pam_certauth/host-id-dump/`, write the TSV, sync, unmount.
///
...
fn write_to_usb(dev: &UsbDevice, filename: &str, content: &str) -> Result<PathBuf, DumpError> {
    ...
    let base = Path::new("/run/pam_certauth/host-id-dump");
...
pub fn run(opts: DumpHostIdOptions) -> Result<DumpDestination, DumpError> {
    let validated = pam_certauth_core::config::load_validated_config(&opts.config)
...
mod tests {
    ...
    fn render_tsv_header_and_format() {
        ...
        assert!(lines[0].starts_with("# pam-certauth host identity probe — atm-001"));
        assert!(lines[0].ends_with("2026-04-27T10:15:30Z"));
        // header row
    ...
    fn run_writes_to_file_when_output_set() {
        ...
        assert!(body.contains("# pam-certauth host identity probe"));
...
"#,
    ...
    fn epoch_to_utc_roundtrip_known_value() {
        // 2026-04-27T10:15:30Z = 1777284930 (verified with `date -u`).
        let (y, mo, d, h, mi, s) = epoch_to_utc(1_777_284_930);
        assert_eq!((y, mo, d, h, mi, s), (2026, 4, 27, 10, 15, 30));
