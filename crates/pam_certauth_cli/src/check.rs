...
pub struct CheckArgs {
    ...
    /// matching the daemon's default so a bare `pam-certauth check` covers
...
pub fn run(args: CheckArgs) -> ExitCode {
    let validated = match pam_certauth_core::config::load_validated_config(&args.config) {
        Ok(v) => v,
        Err(e) => {
            eprintln!(
