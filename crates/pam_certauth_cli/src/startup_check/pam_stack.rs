...
pub fn evaluate_service(path: &Path, text: &str, report: &mut StartupCheckReport) {
    ...
    for (idx, raw) in text.lines().enumerate() {
        ...
        if is_session_pam_certauth(line) {
    ...
    // pam_certauth.so` MUST come AFTER `pam_systemd.so` / @include
    ...
    match (session_certauth_line, systemd_anchor_line) {
        (Some(ses), Some(sys)) if ses < sys => {
            report.push(StartupCheckRecord::error(
                ...
                format!(
                    "PAM session-phase misorder in {path}: 'session required pam_certauth.so' \
        ...
        (Some(_), Some(_)) => {
            report.push(StartupCheckRecord::info(
                ...
                format!(
                    "PAM stack {path}: 'session required pam_certauth.so' correctly placed \
                     after pam_systemd.so / @include common-session",
                    path = path.display()
                ),
            ));
        }
        // session pam_certauth present but no systemd anchor: host has no
        ...
        (Some(_), None) => {
            report.push(StartupCheckRecord::info(
                ...
                format!(
                    "PAM stack {path}: 'session required pam_certauth.so' present but no \
        ...
        // No session pam_certauth line at all → service not integrated for
...
fn is_session_pam_certauth(line: &str) -> bool {
    // Matches `session <control> pam_certauth.so [args...]` regardless of
