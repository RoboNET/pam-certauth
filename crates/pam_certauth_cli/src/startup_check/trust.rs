//! Trust anchor / intermediate file checks + `/etc/pam_certauth/ca/`
//! permission sanity.
...
/// Surface world-writable `/etc/pam_certauth/ca/`. Not fatal — production
...
pub fn check_ca_dir_permissions(fs_root: Option<&Path>, report: &mut StartupCheckReport) {
    ...
    let ca_dir = root.join("etc/pam_certauth/ca");
