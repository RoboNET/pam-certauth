...
impl LogindActionsTrait for NoopActions {
    async fn lock_session(&self, id: &str) -> anyhow::Result<()> {
        tracing::info!(target: "pam_certauth.monitord", id, "noop lock_session");
        Ok(())
    }
    async fn terminate_session(&self, id: &str) -> anyhow::Result<()> {
        tracing::info!(target: "pam_certauth.monitord", id, "noop terminate_session");
        Ok(())
    }
    async fn power_off(&self) -> anyhow::Result<()> {
        tracing::info!(target: "pam_certauth.monitord", "noop power_off");
        Ok(())
    }
}
