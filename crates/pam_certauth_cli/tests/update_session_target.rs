...
use pam_certauth_cli::actions::spawn_action_runner;
use pam_certauth_cli::logind::LogindActionsTrait;
use pam_certauth_cli::registry::{ActiveSession, RegistryStore, SessionRegistry};
use pam_certauth_cli::state::{ActionRequest, OnUsbRemoved};
use pam_certauth_cli::testing::{spawn_test_server_with, RecordingActions};
use pam_certauth_cli::udev_query::AlwaysPresent;
...
async fn update_session_target_unknown_id_returns_bad_request() {
    ...
    match err {
        pam_certauth_core::error::IpcError::Server { code, .. } => {
            assert_eq!(code, pam_certauth_proto::error_codes::BAD_REQUEST);
        }
